...
const UNLOCK_THRESHOLDS = [0, 10, 24, 45, 70];
...
const state = {
  ...
  offsetSum: 0,
  offsetCount: 0,
  previewSpeedCurrent: PREVIEW_SPEED,
  cameraZoom: 1,
  cameraTargetZoom: 1,
  cameraYOffset: 0,
...
function startGame() {
  ...
  state.previewName = CAT_NAMES[0];
  state.score = 0;
  state.wobble = 0;
  state.wobbleTarget = 0;
  state.wobbleDangerFrames = 0;
  state.collapseTimer = 0;
  state.lastUnlockedIndex = 0;
  state.ladderAnimating = false;
  state.hearts = 5;
  state.centerOfGravity = null;
  state.balanceOffset = 0;
  state.offsetSum = 0;
  state.offsetCount = 0;
  state.previewSpeedCurrent = PREVIEW_SPEED;
  state.cameraZoom = 1;
...
function pickRandomCat() {
  const available = state.unlockedCats.filter((name) => assetCache.has(name));
  if (!available.length) {
    return CAT_NAMES[0];
  }
...
function placeInitialCat() {
  ...
  state.centerOfGravity = x + cat.width / 2;
  state.balanceOffset = 0;
  state.offsetSum = 0;
  state.offsetCount = 1;
}
...
function attemptDrop() {
  ...
  const name = state.previewName || pickRandomCat();
  const metadata = getCatMetadata(name);
  const image = assetCache.get(name);
...
function updatePreview() {
  if (state.activeCat || state.mode !== "running") return;
  const desiredBoost = Math.min(3.2, Math.max(0, (state.stack.length - 1) * 0.06));
  const desiredSpeed = PREVIEW_SPEED + desiredBoost;
  state.previewSpeedCurrent += (desiredSpeed - state.previewSpeedCurrent) * 0.09;
  const driftSpeed = state.previewSpeedCurrent / Math.max(0.55, state.cameraZoom);
  state.previewX += state.previewDirection * driftSpeed;
...
function updateActiveCat() {
  ...
  const accel = GRAVITY_BASE + Math.min(1.4, state.stack.length * 0.05);
  const fallMultiplier = Math.min(2, 1 + state.stack.length * 0.03);
  cat.vy += accel * fallMultiplier;
  cat.y += cat.vy * fallMultiplier;
...
function getPreviewBounds() {
  ...
  return {
    left: bounds.left + CAT_WIDTH / 2 - 8,
    right: bounds.right - CAT_WIDTH / 2 + 8
  };
}
...
function drawBackground() {
  ...
  gradient.addColorStop(0, "#fff1f3");
  gradient.addColorStop(0.55, "#fde6f1");
  gradient.addColorStop(1, "#fef6e8");
...
function updateUnlocks() {
  const unlockedCount = UNLOCK_THRESHOLDS.reduce((count, threshold) => {
    return state.stack.length >= threshold ? count + 1 : count;
  }, 0);
  const nextIndex = Math.min(CAT_NAMES.length - 1, unlockedCount - 1);
  if (nextIndex > state.lastUnlockedIndex) {
    triggerLadderAdvance();
    state.hearts = 5;
    renderHearts();
    state.lastUnlockedIndex = nextIndex;
  }
  state.unlockedCats = CAT_NAMES.slice(0, Math.max(1, unlockedCount));
  renderUnlockList();
}
...
function updateUnlockFill() {
  ...
  const unlockedIndex = state.lastUnlockedIndex;
  const nextIndex = Math.min(UNLOCK_THRESHOLDS.length - 1, unlockedIndex + 1);
  const prevThreshold = UNLOCK_THRESHOLDS[unlockedIndex] || 0;
  const nextThreshold = UNLOCK_THRESHOLDS[nextIndex] || prevThreshold + 1;
  const span = Math.max(1, nextThreshold - prevThreshold);
  const progress = Math.min(1, Math.max(0, (state.stack.length - prevThreshold) / span));
  const fill = progress * 100;
  unlockFill.style.height = `${fill}%`;
  const color = CAT_COLORS[unlockedIndex] || "#f6b35e";
