...
## Updates (cosmetic shop + points)
- Added a “Visit Shop” button to the main menu that opens a dedicated overlay with a category row and a placeholder message for now. The shop displays cached shop points and keeps the overlay accessible via the shop button or close control, while still reusing the existing modal styling so the background game screen stays dimmed.
- Introduced `shopPoints` backed by `localStorage`, earned by every survival-mode result (loss or victory). Points now update whenever the run ends and persist across reloads, audit the stored value on load, and keep showing on both the main menu and within the shop overlay.
- Created `CodexOutput/docs/platform-prompts.md` with eight generative prompts to help design new platform skins that match the stacker canvas (doc stored for future asset creation).

## Tests (cosmetic shop + points)
- `cd /Users/garrettjohnson/Desktop/Web Games/CatStacker && npm run start >/tmp/catstacker.log 2>&1 & server_pid=$!; sleep 1; node /Users/garrettjohnson/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js --url http://127.0.0.1:4173/index.html --click-selector "#mode-survival" --actions-file /Users/garrettjohnson/.codex/skills/develop-web-game/references/action_payloads.json --iterations 3 --pause-ms 250 --screenshot-dir CodexOutput/tests; node /Users/garrettjohnson/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js --url http://127.0.0.1:4173/index.html --click-selector "#mode-checkpoint" --actions-file /Users/garrettjohnson/.codex/skills/develop-web-game/references/action_payloads.json --iterations 3 --pause-ms 250 --screenshot-dir CodexOutput/tests; kill $server_pid` (passes; shop points now persist and the overlays stay responsive; artifacts landed in `CodexOutput/tests`). 
